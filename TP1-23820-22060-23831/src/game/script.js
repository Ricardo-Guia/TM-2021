...
highscore = localStorage.getItem("teste") == null ? 0 :
...
function preload() {
    ...
    this.load.image("lifecounter", "https://raw.githubusercontent.com/kudchikarsk/phaser-pacman/master/assets/images/pac%20man%20life%20counter/spr_lifecounter_0.png");
     this.load.image("cereja","https://raw.githubusercontent.com/kudchikarsk/phaser-pacman/master/assets/images/pac%20man%20cherry/spr_cherry_0.png")
...
function create() {
    ...
    this.anims.create({
        ...
        frames: this.anims.generateFrameNumbers(spritesheet, { start: 3, end: 3 }),
        frameRate: 10,
        repeat: -1
    });
    ...
    this.anims.create({
        key: Animation.Ghost.DarkBlue.Move,
        frames: this.anims.generateFrameNumbers(spritesheet, { start: 2, end: 2 }),
        frameRate: 10,
        repeat: -1
    });
    ...
    cerejas= this.physics.add.group();

    let cerejaIMG = this.add.sprite(76,48,"cereja");
    cerejaIMG.setScale(0.6);
    cerejas.add(cerejaIMG);

    this.physics.add.overlap(player.sprite,cerejas,function(){
        cerejaIMG.destroy(true);
        ...
        function(sprite, pill) {
            ...
            if (pillsCount == pillsAte) {
                ...
                if(nivel ==2){
                    ...
                    let position = new Phaser.Geom.Point(45 ,48);
                    let ghost = new Ghost(scene, position, Animation.Ghost.White);
                    ghosts.push(ghost);
                    ghostsGroup.add(ghost.sprite);

                    let cerejaIMG = this.add.sprite(700,48,"cereja");
                    cerejaIMG.setScale(0.6);
                    cerejas.add(cerejaIMG);
                    this.physics.add.overlap(player.sprite,cerejas,function(){
                        cerejaIMG.destroy(true);
                ...
                if(nivel == 3){
                    let position = new Phaser.Geom.Point(750 ,15);
                    let ghost = new Ghost(scene, position, Animation.Ghost.DarkBlue);
                    ghosts.push(ghost);
                    ghostsGroup.add(ghost.sprite);

                    let cerejaIMG = this.add.sprite(400,250,"cereja");
                    cerejaIMG.setScale(0.6);
                    cerejas.add(cerejaIMG);
                    this.physics.add.overlap(player.sprite,cerejas,function(){
                        cerejaIMG.destroy(true);
    ...
    highscoreText = this.add
        .text(140,595,"Highscore: " + highscore)
        .setFontFamily("Arial")
        .setFontSize(18)
        .setColor("#ffffff");
...
function newGame() {
    ...
    if(player.score> localStorage.getItem("teste")){
       localStorage.setItem("teste",player.score);
   }
...
class Ghost {
    constructor(scene, position, anim) {
        ...
        this.anim = anim;
       if(nivel == 1) this.speed = 100;
        if(nivel == 2) this.speed = 125;
        if(nivel >= 3 ) this.speed = 150;
